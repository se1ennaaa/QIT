...
part 'manager_providers.freezed.dart';

@freezed
class ManagerListState with _$ManagerListState {
  const factory ManagerListState({
    @Default(AsyncValue.loading()) final AsyncValue<List<ManagerModel>> list,
    @Default(ManagerParamsModel()) final ManagerParamsModel params,
    @Default(false) final bool isPageNotFound,
  }) = _ManagerListState;

  factory ManagerListState.initial() => ManagerListState();
}

@riverpod
class ManagerList extends _$ManagerList {
  @override
  ManagerListState build({ManagerParamsModel? params}) {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      if (params != null) state = state.copyWith(params: params);
      fetchManagerList(params);
    });

    return ManagerListState();
  }

  void fetchManagerList(ManagerParamsModel? params) async {
    final response =
        await ref.read(managerRepoProvider).fetchManagerList(state.params);
  ...
  void filter(ManagerParamsModel params) async {
    state = state.copyWith(
  ...
  void loadMore() async {
    ...
    state = state.copyWith(
        list: const AsyncValue<List<ManagerModel>>.loading()
  ...
  void refresh() async {
    final response = await ref
        .read(managerRepoProvider)
        .fetchManagerList(state.params.copyWith(page: 1));
    if (response.isSuccessful) {
      state = state.copyWith(
        list: AsyncValue.data(response.result!),
        isPageNotFound: false,
...
@riverpod
class ManagerDetails extends _$ManagerDetails {
  @override
  Future<ManagerDetailsModel> build(int id) async {
    final response =
        await ref.read(managerRepoProvider).fetchManagerDetails(id);
