...
part 'sewing_equipment_providers.g.dart';
part 'sewing_equipment_providers.freezed.dart';

@freezed
class SewingEquipmentListState with _$SewingEquipmentListState {
  const factory SewingEquipmentListState({
    @Default(AsyncValue.loading())
    final AsyncValue<List<SewingEquipmentModel>> list,
    @Default(SewingEquipmentParamsModel())
    final SewingEquipmentParamsModel params,
    @Default(false) final bool isPageNotFound,
  }) = _SewingEquipmentListState;

  factory SewingEquipmentListState.initial() => SewingEquipmentListState();
}

@riverpod
class SewingEquipmentList extends _$SewingEquipmentList {
  @override
  SewingEquipmentListState build({SewingEquipmentParamsModel? params}) {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      if (params != null) state = state.copyWith(params: params);
      fetchSewingEquipmentList(params);
    });

    return SewingEquipmentListState();
  }

  void fetchSewingEquipmentList(SewingEquipmentParamsModel? params) async {
    final response = await ref
        .read(sewingEquipmentRepoProvider)
        .fetchSewingEquipmentList(state.params);
  ...
  void filter(SewingEquipmentParamsModel params) async {
    state = state.copyWith(
  ...
  void loadMore() async {
    ...
    state = state.copyWith(
        list: const AsyncValue<List<SewingEquipmentModel>>.loading()
  ...
  void refresh() async {
    final response = await ref
        .read(sewingEquipmentRepoProvider)
        .fetchSewingEquipmentList(state.params.copyWith(page: 1));
    if (response.isSuccessful) {
      state = state.copyWith(
        list: AsyncValue.data(response.result!),
        isPageNotFound: false,
...
@riverpod
class SewingEquipmentDetails extends _$SewingEquipmentDetails {
  @override
  Future<SewingEquipmentDetailsModel> build(int id) async {
    final response = await ref
        .read(sewingEquipmentRepoProvider)
        .fetchSewingEquipmentDetails(id);
