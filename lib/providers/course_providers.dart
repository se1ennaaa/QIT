...
part 'course_providers.freezed.dart';

@freezed
class CourseListState with _$CourseListState {
  const factory CourseListState({
    @Default(AsyncValue.loading()) final AsyncValue<List<CourseModel>> list,
    @Default(CourseParamsModel()) final CourseParamsModel params,
...
class CourseList extends _$CourseList {
  @override
  CourseListState build({CourseParamsModel? params}) {
  ...
  void fetchCourseList(CourseParamsModel? params) async {
    final response =
        await ref.read(courseRepoProvider).fetchCourseList(state.params);
  ...
  void filter(CourseParamsModel params) async {
    state = state.copyWith(
  ...
  void loadMore() async {
    ...
    state = state.copyWith(
        list: const AsyncValue<List<CourseModel>>.loading()
  ...
  void refresh() async {
    final response = await ref
        .read(courseRepoProvider)
        .fetchCourseList(state.params.copyWith(page: 1));
    if (response.isSuccessful) {
      state = state.copyWith(
        list: AsyncValue.data(response.result!),
        isPageNotFound: false,
...
@riverpod
class CourseDetails extends _$CourseDetails {
  @override
  Future<CourseDetailsModel> build(int id) async {
    final response = await ref.read(courseRepoProvider).fetchCourseDetails(id);
