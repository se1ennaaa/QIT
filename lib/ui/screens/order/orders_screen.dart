...
class OrdersScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final initialParams =
        OrderParamsModel(isMy: isMy, isFavorites: isFavourite);
    final orderListState = ref.watch(orderListProvider(params: initialParams));
    final scrollController = useScrollController();
    final categoryListState =
        ref.watch(categoryListProvider(CategoryType.order));
    ...
    scrollController.addListener(
      () {
        if (scrollController.position.pixels >=
          ...
          ref
              .read(orderListProvider(params: initialParams).notifier)
              .loadMore();
        }
        // NavBarVisible
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () {
                            if (item.id != orderListState.params.categoryId) {
                              ref
                                  .read(orderListProvider(params: initialParams)
                                      .notifier)
                                  .filter(
                                    OrderParamsModel(
                                      categoryId: item.id,
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              color: item.id == orderListState.params.categoryId
                                  ? const Color(0xFF9605AC)
                                  : const Color(0xFFF6EFF7),
                              border:
                                  item.id != orderListState.params.categoryId
                                      ? Border.all(
                                          width: 1,
                                ...
                                if (item.title != null)
                                  Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 10,
                                      color: item.id ==
                                              orderListState.params.categoryId
                                          ? Colors.white
                                          : const Color(0xFF9605AC),
                                    ),
                                  ),
                ...
                data: (data) {
                  ...
                  return SliverToBoxAdapter(
                    child: OrdersSection(
                      canEdit: isMy,
                      orderList: data,
                      paginationLoading: orderListState.list.isLoading,
                      onFavoriteTap: (id) {
