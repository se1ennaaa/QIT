...
class MadinaMarketDetailsScreen extends HookConsumerWidget {
  const MadinaMarketDetailsScreen(this.id,
      {super.key, this.listInitialParams, this.canEdit = false});

  final int id;
  final MadinaContainerParamsModel? listInitialParams;
  final bool canEdit;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final madinaMarketDetailsState =
        ref.watch(madinaContainerDetailsProvider(id));
      ...
      child: Scaffold(
        body: ListView(
          ...
          children: [
            CustomAppBar(
              title: 'Мадина рынок',
              isFavorite: madinaMarketDetailsState.maybeWhen(
                orElse: () => false,
              ...
              onHeartTap: () {
                ...
                ref.read(toggleFavoriteProvider(id, FavoriteType.markets));
                ref
                    .read(madinaContainerDetailsProvider(id).notifier)
                    .toogleFavorite();
                if (listInitialParams != null) {
                  ref
                      .read(
                          madinaContainerListProvider(params: listInitialParams)
                              .notifier)
                      .toogleFavorite(id);
                }
              },
            ),
            madinaMarketDetailsState.when(
              data: (data) => Column(
                children: [
                  Gap(27),
        ...
        floatingActionButton: madinaMarketDetailsState.maybeWhen(
          orElse: () => null,
          data: (data) {
            ...
            return Padding(
              ...
              child: EditAdButtonScreen(
                onTap: () async {
                  showAddMadinaContainerSheet(context, id: id);
                },
                height: 49,
                borderRadius: 15,
                titleFontSize: 14,
